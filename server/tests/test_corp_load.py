...
def test_FN1_seed_populates_mapping_at_last_step(seeded_db):
    ...
    assert _scalar(seeded_db, "SELECT COUNT(*) FROM TCOMPANY_CORP") == len(mapped) == 149
    assert _scalar(seeded_db, "SELECT COUNT(*) FROM TCORP") == len({r["corp_code"] for r in mapped}) == 148
...
def test_FN1_reapply_is_idempotent(seeded_db):
    ...
    assert stats["mapped"] == 149 and stats["unmatched"] == []
