...
def create_app() -> FastAPI:
    ...
    # 운영 콘솔(SP-AUTH-19 — SSH 터널 · 관리 호스트) — **M9 + 화이트리스트 둘 다** 있어야 등록한다.
    #
    ...
    #
    # ⚠ 노출 범위는 이 조건이 아니라 `deps.require_console_access` 가 지킨다. 설정을 켠 것과
    #   인터넷에 연 것은 다른 사건이고, 후자는 nginx 를 거친 요청을 404 로 끊어 막는다 — 예외는
    #   비밀번호 뒤의 관리 호스트 하나뿐이고 그것도 `ADMIN_HOST`·`ADMIN_GATE_SECRET` 이 있을 때만이다.
