...
-- ============================================================================
-- SC15 커뮤니티 4테이블 — SP-DB-18 정본 DDL (M9 참여 그룹, 2026-08-27)
-- FK 부모→자식 생성순서: TPOST → TPOST_COMMENT → TPOST_REACTION → TPOST_REPORT
--   (부모 TMEMBER·TCOMPANY 는 위에서 이미 생성됨).
-- 🚫 **조회수 컬럼(VIEW_CNT)을 두지 않는다.** 봇 트래픽이 실사용의 수백 배(`GET /` 하루
--   1,875~11,036 vs 실세션 1~4)라 조회수는 곧 거짓 숫자가 된다 — 함정 (57)("대체 콘텐츠가
--   집계인 척") 과 같은 계열. 정렬은 최신·댓글·좋아요 3종뿐(FR-121).
-- 삭제는 소프트(STATUS_CD) + 본문 마스킹. 운영자 조치는 hidden(신고 콘솔). 하드 삭제 라우트 없음.
-- 작성자 식별은 MBR_ID 뿐(닉네임은 TMEMBER 조인 표시, INV-8). 탈퇴·회사 삭제 시 SET NULL 존치
--   = TBENEFIT_EDIT_LOG 와 같은 보존 규약.
-- ============================================================================
