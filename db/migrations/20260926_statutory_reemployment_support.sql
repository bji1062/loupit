...
-- 선례: db/migrations/20260920_remove_statutory_phrases.sql (같은 형식 · 같은 가드)
--
...
--     대상이 넓은(만 50세 이상 임직원) 영역 선택 진로설계교육만
--   · 한국항공우주산업 retirement_support — 창업 · 재취업 · 재무를 걷고 정년퇴임 행사 · 여행 · 귀농 지원만
...
-- 한국항공우주산업 retirement_support — 정년퇴임 행사 · 여행 · 귀농 지원만
SET @c = (SELECT COMP_ID FROM TCOMPANY WHERE COMP_ENG_NM = 'kai');
UPDATE TCOMPANY_BENEFIT
   SET QUAL_DESC_CTNT = '정년퇴직자 지원 (복리후생제도 페이지 동기부여 항목, 정년퇴임 행사 사진 게재). 인사제도 페이지의 퇴직예정자 지원 제도 중 여행·귀농 지원 — 지원 금액·기간 미기재'
 WHERE COMP_ID = @c AND BENEFIT_CD = 'retirement_support' AND BADGE_CD = 'official'
