-- ══════════════════════════════════════════════════════════════════════
-- 공식 출처 대조로 드러난 금액 오류 정정 — 초기 등록 4개사 8행
...
--
-- 무엇을 바꾸나:
--   NAVER stock_grant 1,000 → 금액 없음 · 이름 주식 지급(Stock Grant). 2025 통합보고서 p.205 2025년 이사회
--     결의 내용 표에 Stock Grant 지급을 위한 자사주 처분 안건이 있다(회차와 날짜는 PDF 표 추출로는 확정하지
--     못했다) — 제도는 있으나 대상과 금액은 현행 공식 출처(채용 사이트 · 보고서)에 없다.
--     전 직원 · 1,000만원은 근거가 없어 걷는다.
--     (p.108 의 2019~2021 스톡옵션은 다른 제도다 — 처음 대조가 이 둘을 혼동해 삭제로 판정했었다.)
...
--     채용 사이트의 허먼밀러 에어론 의자 기본 제공 · 희망 시 스탠딩 데스크는 설명에 남긴다.
...
-- 멱등: 문마다 지금 금액을 WHERE 에 건다(1000·40·100·360·24·290·130·200) — 두 번째 실행은 0행.
...
-- 기대: 8문 각각 Rows matched 1 Changed 1. 두 번째 실행은 전부 0. 행 수는 변하지 않는다.
...
-- 주식 부여 — Stock Grant 는 있으나(2025 이사회 결의) 대상·금액은 공식 출처에 없음
UPDATE TCOMPANY_BENEFIT
   SET BENEFIT_NM     = '주식 지급(Stock Grant)',
       BENEFIT_AMT    = NULL,
       QUAL_YN        = TRUE,
       AMT_SOURCE_CD  = 'none',
       NOTE_CTNT      = NULL,
       QUAL_DESC_CTNT = '자사주로 주식을 지급하는 Stock Grant — 2025년 이사회 결의'
...
UPDATE TCOMPANY_BENEFIT
   SET BENEFIT_AMT    = NULL,
       ...
       AMT_SOURCE_CD  = 'none',
       NOTE_CTNT      = NULL,
       QUAL_DESC_CTNT = '입사 시 최대 360만원 장비 예산, 이후 매월 추가 예산 지원(직군별로 다름) · 허먼밀러 에어론 의자 기본 제공, 희망 시 스탠딩 데스크'
...
UPDATE TCOMPANY_BENEFIT
   SET BENEFIT_AMT    = 67,
       ...
       NOTE_CTNT      = '만 3년 근무마다 1개월 유급휴가와 휴가비 200만원 — 휴가비를 연 67만원으로 환산한 값'
