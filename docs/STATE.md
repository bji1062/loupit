...
| **히트맵(SC16 신설)** | `/heatmap` 복지·실적 히트맵 — KRX 업종(종목코드 자동 매칭 100/102)·squarify 정적 배치·가로/세로·7단계 색·상단 탭 `히트맵`. [SP-HEAT](SPEC/16-히트맵.md) · **라이브(PR #12)** + **카테고리 모드**(9 카테고리 × 항목 묶음 × 회사, 색 = 출처 공식/추정/정성 — 정량 645건 중 공식 73건이라 금액 색 대신 출처 색) TASK/16 11/12(배포 대기). 비교판 3종(대분류 10 / 업종 67 / KRX)을 실데이터로 만들어 KRX 로 확정 | 2026-08-27 |
