...
| **히트맵(SC16 신설)** | `/heatmap` 복지·실적 히트맵 — KRX 업종(종목코드 자동 매칭 100/102)·squarify 정적 배치·가로/세로·7단계 색·상단 탭 `히트맵`. [SP-HEAT](SPEC/16-히트맵.md) · TASK/16 7/8(남은 1 = 배포). 비교판 3종(대분류 10 / 업종 67 / KRX)을 실데이터로 만들어 KRX 로 확정 | 2026-08-27 |
