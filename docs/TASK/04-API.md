...
- [v] T-04.2.1 config.py Settings (환경변수·CORS 목록·캐시 TTL, 인증키 부재)
...
- [v] T-04.2.2 database.py aiomysql 풀·읽기 헬퍼 (fetch_all/fetch_one, %s 바인딩, 쓰기 헬퍼 부재)
...
- [v] T-04.2.3 cache.py TTLCache (get/set/clear, monotonic 만료)
...
- [v] T-04.4.1 build_reference_bundle(conn)->dict 5쿼리 조립 (런타임·generator 공유 단일 소스)
...
- [v] T-04.6.1 GET /health 라이브니스 200 (TH-1)
...
- 선결정 DG-4(로컬 — 전역 레지스트리 00 §4 DG-4): `/health` 레디니스 503 확장(`database.ping()`→False 시 503 `{"status":"degraded"}`) 구현 여부 — MVP 라이브니스 전용(SPEC 기본) vs 레디니스 확장(FR-91 대안). **확정(2026-07-11, 00 §4): 미채택** — MVP는 라이브니스(T-04.6.1) 전용으로 확정. 아래 T-04.6.2는 구현 대상에서 제외한다(스킵 아님 — 애초에 코드·테스트 작성 대상이 아님. `database.ping()` 헬퍼 자체는 T-04.2.2에 존재하나 라우터에서 미사용).
- [ ] T-04.6.2 health 레디니스 확장 503 (TH-2) [DG-4 미채택 — 구현 대상 아님, 확정 2026-07-11]
  - 구현: (선택, FR-91 대안) `health.py` — `database.ping()`이 `False`면 `JSONResponse(status_code=503, content={"status":"degraded"}, headers={"Cache-Control":"no-store"})`. **DG-4 미채택으로 구현하지 않음** — MVP는 라이브니스(T-04.6.1)만 제공.
  - 테스트: TH-2 — **작성하지 않음**(대상외). 향후 DG-4 재논의 시 재개.
  - refs: SP-API-8 · FR-91 · NFR20 · DG-4

## T-04.7 GET /api/v1/reference/all + 인메모리 캐시  — (SP-API-9·4, FR-92·FR-D1, INV-2, NFR3)
- [v] T-04.7.1 reference/all 캐시 미스 조립·직렬 바이트 반환 (TR-1) (Tier0)
...
- [v] T-04.7.2 reference/all 전송 헤더 (TR-2) (Tier0)
...
- [v] T-04.7.3 프로파일러 키 부재 (TR-3) (Tier0)
...
- [v] T-04.7.4 reference/all 스키마 준수·회사 필수 배열 (TR-4)
...
- [v] T-04.7.5 인메모리 캐시 1회 조립 (TR-5) (Tier0)
...
- [v] T-04.8.1 companies/search 이름·별칭 LIKE·축소 5필드 투영 (TSE-1)
...
- [v] T-04.8.2 q 검증 매트릭스 — 미제공 422·빈/공백 200[]·과길이 422 (TSE-2·3·4)
...
- [v] T-04.8.3 LIMIT 20 상한 + 0건 매칭 빈 배열 (TSE-5·TSE-7)
...
- [v] T-04.8.4 주입·LIKE 와일드카드 이스케이프 (TSE-6)
...
- [v] T-04.9.1 companies/{comp_id} 완전 회사 객체 (TC-1)
...
- [v] T-04.9.2 미존재 404·비정수/<1 검증 422 (TC-2·3·4)
...
- [v] T-04.10.1 전역 예외 핸들러 500 (스택/SQL/내부경로 미노출) (TE-1)
