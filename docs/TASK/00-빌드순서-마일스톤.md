...
> **M9 표면 게이트 전이(#7 → #28)**: MVP #7(TS-1: 익명 표면·쓰기 라우트 0)은 M9에서 **T-13.1.2(AU-1)**가 참여 쓰기 라우트 **열거집합**을 포함하도록 확장 재명세한다(TS-1 어서션이 '쓰기 0'→'열거집합 외 쓰기 0'; INV-1도 SC14 기여 쓰기 카브아웃). #7과 #28은 같은 표면 계약의 MVP·M9 버전이며, M9 이후 표면 회귀 게이트는 AU-1이 정본이다. (표면의 익명 GET 카운트 자체는 별도 드리프트로 본 개정 범위 밖.)
