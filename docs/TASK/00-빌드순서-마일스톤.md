...
> **이 파일이 소유하는 것(정본)**: 마일스톤(M0~M8 MVP + **M9 SC14 참여 증분**)·도메인 간 의존 DAG·크리티컬 패스·결정 게이트(DG-1~5)·Tier-0 회귀 게이트 맵·릴리스 게이트(G1~G4)·릴리스 순서(SP-ARCH-9). **리프 상세(구현·테스트 계약)는 소유하지 않는다** — 그건 각 도메인 TASK `01~12`가 소유한다. 본 문서는 그 12개를 **언제·어떤 순서로·무엇을 게이트 삼아** 엮을지만 확정한다.
...
빌드는 M0~M8(MVP) 9개 마일스톤으로 흐르고, **M9(참여·로그인)는 라이브(M8) 이후의 SC14 증분**이다. 각 마일스톤 = "게이트 케이스가 green이 되는 단위". 게이트가 깨지면 다음 마일스톤 진입 금지.
...
| **M2** API·번들 | SP-API | `build_reference_bundle` 단일 소스·Pydantic 모델·익명 GET 5종 + 익명 비교 로그 POST 1종·인메모리 TTL 캐시·CORS·표면 가드 | M1 · **DG-4**(T-04.6.2 선택 리프 한정) | TS-1·TS-2·TR-1~3·TR-5 | - [v] (2026-07-11 완료: 166 pass) |
| **M3** 계산엔진 | SP-ENGINE | `calc.js` 전 export·밴드(DEC-2)·순수 ES모듈 | M1 데이터 계약(필드명)만 · 러닝 API 무의존 → **M2와 병렬** · DG-1 | T-ENGINE-26·41·45 | - [v] |
| **M4** 광고·정책 기반 | SP-ADS + SP-POL | `adsConfig`·`adPolicy`/`mountAds`·`affiliate.json`·동의 배너 + 정책 4종 문안·정정 경로·푸터 링크 | M0(SP-DS·SP-ARCH) · **SP-FE-9·10 원시**(`dom.js` 안전유틸·`store.js` — M4 전 선행, §2) | UT-ADS-GATE-1·PC-13 | - [-] (SP-POL 18/18·SP-ADS 코드완료·게이트 green; MB-ADS-* 수동 QA·실 client id는 M8) |
...
| **M6** 프론트 SPA | SP-FE | 셸·라우팅·`boot`/REF·검색·입력·`normalizeCompany`·엔진 어댑터·`report` 렌더·`store`·이스케이프 | M3(SP-ENGINE)·M2(SP-API)·M4(SP-ADS)·M0(SP-DS) | UT-ESC-1~3 | - [-] (코드 35/46+셸 완료·프론트 자동 테스트 green; 잔여 10건 MB-* 수동 QA는 M8) |
...
| **M9** 참여·로그인(SC14 증분) | **SP-AUTH** + SP-DB(참여 7테이블·`verified`·`_CNT`) + SP-INFRA(`loupit_mail` 존·`SMTP_*`) | member/employment/benefit_edit 라우터·session/auth_code/employment/benefit_edit 서비스·`require_member`/`require_employment` 의존성·mailer(Console/SMTP)·`ops` CLI·config 확장·참여 7테이블 DDL·nginx `loupit_mail` 존 | M8(라이브) · **DG-5**(도메인 화이트리스트) · **T-13.2.1 선행**(백업 확장) · **SP-INFRA `loupit_mail`·`SMTP_*` 재명세** · **SP-POL 정책 SC14 반영 재생성** | **AU-1~4**(#28~31) + AL·AS·AM·AE·AB·AH·AC·AX·AO | - [ ] |
...
graph TD
  ...
  classDef incr fill:#1e3a8a,color:#ffffff,stroke:#1e40af,stroke-width:2px;
  class AUTH incr;
```
...
- **M9(SP-AUTH·참여)**: 크리티컬 패스 **외부**의 **라이브(M8) 이후 별도 증분 축**이다(선행 SP-DB·SP-API·SP-INFRA). MVP 배포 경로와 독립적으로 진행되며, 로그인 라이브 노출은 AdSense 심사 후.
...
| --- | --- | --- | --- | :---: | --- | :---: |
| **DG-1** (OI-3) | 카테고리별 만료 TTL 값 (후보: compensation 6·perks 9·time_off/flexibility 12·work_env/health/growth/leisure/family 18개월) | `EXPIRES_DTM` 백필 상수 + 만료 밴드 `+0.15` 판정 입력 | SP-SEED T-03.5.5(SB-8·9) · SP-ENGINE T-05.6.2(T-ENGINE-24·25) | M1·M3 | 착수 전 AskUserQuestion로 TTL 표 확정 | - [v] (2026-07-11 확정) |
| **DG-2** (OI-6) | `amt_source` 백필 판별 규칙 (기존 `(추정)`/`환산` 표기 → stated/estimated/none) | DEC-2 백필의 `AMT_SOURCE_CD` 도출 규칙 | SP-SEED T-03.5.2(SB-2·3·4·5·7) | M1 | AskUserQuestion로 판별 규칙 확정 | - [v] (2026-07-11 확정) |
| **DG-3** | 엔씨소프트 시드 메타(별칭·산업·`work_style` — KOSPI/KOSDAQ 200 목록 밖) 확보 | 회사 매핑에서 엔씨 리프 시드 불가 | SP-SEED T-03.4.3(SI-2·3) | M1 | 메타 확보 후 AskUserQuestion 확정 | - [v] (2026-07-11 확정) |
| **DG-4** (OI-4/5/7 + 도메인 로컬) | RESEARCH §8 기타 미결 + 로컬 선결정: SP-ARCH **MySQL 인증 플러그인**(`caching_sha2_password`→`cryptography` 포함 vs `mysql_native_password`) · SP-API **`/health` 레디니스 503 확장**(선택) | 매니페스트 pin·엔드포인트 계약 확정 전 임의판단 금지 | SP-ARCH T-01.2.1 · SP-API T-04.6.2 + RESEARCH §8 관련 리프 | M0(ARCH pin)·M2(API) | 각 리프 착수 전 AskUserQuestion | - [v] (2026-07-11 확정) |
| **DG-5** (SC14) | 재직 인증 **회사 도메인 화이트리스트** 시드(어느 회사=어느 이메일 도메인) | `TCOMPANY_EMAIL_DOMAIN` 매칭 데이터는 추측 불가·큐레이션 필요(도메인 미등록 회사는 수동 승인 폴백) | SP-AUTH T-13.8.4(AE-5)·T-13.8.2(도메인 매칭) | M9 | 착수 전 AskUserQuestion로 회사↔도메인 표 확정 | - [ ] |
...
각 도메인의 `tier0Leaves`를 한 표로 모은다. **하나라도 깨지면 배포 차단(예외 없음)** — `release.sh` 4·7단계가 로컬 강제(SP-TEST-7·SP-ARCH-10·SPEC §5 정합). 총 **31개 리프**(MVP 27 #1~27 + SC14 참여 AU-1~4 #28~31).
...
| 7 | SP-API · M2 | T-04.5.2 | TS-1 | API 표면=익명 GET 5종 + 익명 비교 로그 POST 1종; 그 외 쓰기 0, SC14 기여 쓰기는 M9에서 별도 열거 (INV-1) | pytest+httpx |
...
| 28 | SP-AUTH · M9 | T-13.1.2 | AU-1 | 참여 쓰기 라우트 열거·그 외 쓰기 0 (TS-1 확장, INV-1) | pytest+httpx |
| 29 | SP-AUTH · M9 | T-13.1.3 | AU-2 | 미들웨어=CORS 1종·무세션 미들웨어 (TS-2, INV-9) | pytest+httpx |
| 30 | SP-AUTH · M9 | T-13.3.9 | AU-3 | TMEMBER PII 컬럼=이메일·닉네임 2종 (T9, INV-8) | pytest+pymysql |
| 31 | SP-AUTH · M9 | T-13.3.10 | AU-4 | 인증/세션/재직 테이블 원문 컬럼 부재·`*_HASH_VAL`만 (T9, INV-8·NFR30) | pytest+pymysql |
...
> **M9 표면 게이트 전이(#7 → #28)**: MVP #7(TS-1: 익명 표면 — GET 5종 + 비교 로그 POST 1종, 그 외 쓰기 0)은 M9에서 **T-13.1.2(AU-1)**가 참여 쓰기 라우트 **열거집합**을 포함하도록 확장 재명세한다(TS-1 어서션이 '비교 로그 외 쓰기 0'→'열거집합 외 쓰기 0'; INV-1도 SC14 기여 쓰기 카브아웃). #7과 #28은 같은 표면 계약의 MVP·M9 버전이며, M9 이후 표면 회귀 게이트는 AU-1이 정본이다. (표면의 익명 GET 카운트는 GET 5종으로 정합됨 — 2026-07-14 비교 로그 이후.)
...
| --- | --- | --- |
| **G1** | `pytest server/tests/` | SC·CN·DC(SP-DB) + SD·SI·SB·SM(SP-SEED) + TS·TH·TR·TSE·TC·TM·TN·TCORS·TE(SP-API) + **AU·AL·AS·AM·AE·AB·AH·AC·AX·AO(SP-AUTH, M9)** |
| **G2** | `pytest generator/tests/` | GC-1~26(SP-GEN) + PC-1~13(SP-POL) |
| **G3** | `node --test web/` | T-ENGINE-1~48(SP-ENGINE) + UT-*(SP-FE) + UT-ADS-*(SP-ADS) + UT-TOKEN/CONTRAST-*(SP-DS) + **MT-1~4**(SP-TEST 메타) + **apiSend(SP-AUTH AC, M9)** |
...
> **M9 참여(SC14) 증분 — 라이브(M8) 이후 별도 배포**: (1) `db/schema.sql`에 참여 7테이블 추가 전 **T-13.2.1**(`run_tests.sh` 백업/재주입 확장) 선행 필수 — 공유 스키마 게이트 실행이 실 회원 데이터를 전멸시키지 않게 한다. (2) SP-DB(`BADGE_CD='verified'`·`_CNT`·`TMEMBER` 제거 해제)·SP-INFRA(`loupit_mail` 존·`SMTP_*`) 선행 재명세. (3) G1에 SP-AUTH 스위트, Tier-0 맵에 AU-1~4(#28~31), 결정 게이트 DG-5(도메인 화이트리스트). (4) **로그인 기능 라이브 노출은 AdSense 심사 결과 확인 후 + 정책 페이지 SC14 반영(개인정보 P7·약관 T5 신설·T2/P1 개정, SP-POL) 재생성·게시 후**(별도 승인 게이트).
