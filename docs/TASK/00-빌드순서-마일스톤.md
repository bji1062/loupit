...
| **M1** 데이터 계층 | SP-DB → SP-SEED | `schema.sql` 5테이블·`AMT_SOURCE_CD`·백필 DDL → `reference.sql`·96 복지 재이식(→95 등록)·회사≈95 매핑·프리셋 28·DEC-2 백필·`load.py` | M0 + **DG-1·DG-2·DG-3 해소** | SD-3·DC-2·DC-12·DC-13·SB-1·SB-6 | - [-] (SP-DB 1차 완료, SP-SEED 착수 전) |
| **M2** API·번들 | SP-API | `build_reference_bundle` 단일 소스·Pydantic 모델·GET 4종·인메모리 TTL 캐시·CORS·표면 가드 | M1 · **DG-4**(T-04.6.2 선택 리프 한정) | TS-1·TS-2·TR-1~3·TR-5 | - [ ] |
| **M3** 계산엔진 | SP-ENGINE | `calc.js` 전 export·밴드(DEC-2)·순수 ES모듈 | M1 데이터 계약(필드명)만 · 러닝 API 무의존 → **M2와 병렬** · DG-1 | T-ENGINE-26·41·45 | - [ ] |
| **M4** 광고·정책 기반 | SP-ADS + SP-POL | `adsConfig`·`adPolicy`/`mountAds`·`affiliate.json`·동의 배너 + 정책 4종 문안·정정 경로·푸터 링크 | M0(SP-DS·SP-ARCH) · **SP-FE-9·10 원시**(`dom.js` 안전유틸·`store.js` — M4 전 선행, §2) | UT-ADS-GATE-1·PC-13 | - [ ] |
| **M5** 정적 생성 | SP-GEN | 회사 상세≈95·인기 조합·정책·404·`slug`·Jinja2·SEO·JSON-LD·`sitemap.xml`/`robots.txt`·비-JS 본문 | M2(번들 빌더)·M4(SP-POL·SP-ADS)·M0(SP-DS) | GC-2·GC-10·GC-21 | - [ ] |
| **M6** 프론트 SPA | SP-FE | 셸·라우팅·`boot`/REF·검색·입력·`normalizeCompany`·엔진 어댑터·`report` 렌더·`store`·이스케이프 | M3(SP-ENGINE)·M2(SP-API)·M4(SP-ADS)·M0(SP-DS) | UT-ESC-1~3 | - [ ] |
| **M7** 인프라·집계 | SP-INFRA + SP-TEST(집계 완성·MT-2) | `nginx.conf`·`systemd` 유닛·`mysql.cnf`·`release.sh`·`smoke.sh`·백업 + `run_tests.sh` 집계 완성·MT-1~4 배선 | 전 컴포넌트(M1~M6) | CFG-1~6·SM-1~14·MT-1~4 | - [ ] |
