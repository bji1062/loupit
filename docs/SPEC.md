...
| [SPEC/04-백엔드-API.md](SPEC/04-백엔드-API.md) | SP-API | 슬림 읽기전용 FastAPI(GET 5종+비교 로그 POST 1종)·`build_reference_bundle`(단일 소스)·Pydantic 모델·aiomysql·인메모리 TTL 캐시·CORS/캐시 헤더·무인증·비교 로그 외 무쓰기 |
...
| [SPEC/16-히트맵.md](SPEC/16-히트맵.md) | **SP-HEAT** | `/heatmap` 복지·실적 히트맵(SC16): KRX 업종 시드(`generator/data/krx_sector.csv`)·squarify 배치(`generator/treemap.py`)·가로/세로 두 배치·7단계 색(복지 7분위 / 실적 ±3·10·30%)·JS 는 탭 전환만·무광고·표현 사실만 |
| [SPEC/18-복지검색.md](SPEC/18-복지검색.md) | **SP-FIND** | `/find` 복지검색 탭: 코드 사전 집계·표시명 중복 해소·URL/매칭/정렬·접힘 덱(변형 B)·비-JS 표·테스트 |
...
| **SP-API** | 읽기전용 HTTP 서버 | `main`/`config`/`database`/`cache`/`routers`(GET 5종+비교 로그 POST 1종)/`services.reference`(번들 빌더)/Pydantic 모델 | FR-90~96, FR-D1~D11 |
...
- **Tier 0 회귀 게이트(깨지면 배포 차단)**: T-ENGINE-26(official×estimated=±20%, INV-5)·T-ENGINE-45(순수성 INV-4)·T-ENGINE-41(무크래시), TS-1·2(GET 5종·비교 로그 POST 1종·무인증), TR-1~3·5(번들 계약·프로파일러 키 부재·캐시), SD-3/DC-2(회사 ≈95≠200), SB-1/DC-13(official 승격)·DC-12/SB-6(DEC-2 디커플링), GC-2·10·21(개수·비-JS·XSS), UT-ESC/PC-13(이스케이프), UT-ADS-GATE-1(input 무광고). CI 없음 — `infra/deploy/release.sh` 4·7단계가 로컬 게이트로 강제한다.
...
| FR-90~96(읽기전용 API) | GET 5종+비교 로그 POST 1종 전송 계약·캐시·CORS | SP-API | SP-ARCH·SP-INFRA(프록시) |
