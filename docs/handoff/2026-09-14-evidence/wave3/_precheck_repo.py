...
# 리포 경로 사본(통합 계약 규칙 6): 글롭 = 리포의 웨이브 3 12파일, slug 충돌은 나머지 코퍼스와
WAVE3=["포스코인터내셔널","포스코퓨처엠","키움증권","삼성증권","SK바이오팜","대한전선","에스티팜","로보티즈","루닛","씨젠","더존비즈온","가온전선"]
TARGETS=sorted(f"db/seed/benefit/sql/{n}.sql" for n in WAVE3)
...
vocab=set(re.findall(r"^\| `([a-z_0-9]+)`", open("docs/handoff/2026-09-14-evidence/_VOCAB.md",encoding="utf-8").read(), re.M))
existing={}
for f in glob.glob("db/seed/benefit/sql/*.sql"):
    if f in TARGETS: continue
    m=re.search(r"VALUES\s*\('([^']+)',\s*'([^']+)'", open(f,encoding="utf-8").read())
    if m: existing[slug_of(m.group(1))]=m.group(2)
print("corpus (excluding wave3 12):", len(existing))
...
print("TOTAL rows", total, "=> SD-4 would be", 2032+total)
print("\nNEW CODES (not in vocab 86):")
