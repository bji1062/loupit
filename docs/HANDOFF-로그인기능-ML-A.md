...
> **다음 세션 시작점**: 아래 §진행현황의 **Step 8(TASK — M9·게이트 AU-1~4·TASK/13)** 부터. 그 뒤 §D 미결 일관성 스윕. 이 문서 + 메모리 `loupit-login-feature-docs`가 재개 컨텍스트다.
> **최종 갱신**: 2026-07-21, ML-A 7(SPEC/09 약관 T5·T2·개인정보 P7) 저술·적대적 검증(blocker 1·major 1·minor 3 수정) 완료 시점.
...
| 8 | `TASK/00-빌드순서-마일스톤.md`(M9·게이트 AU-1~4) + `TASK/13-로그인참여.md`(신규 리프) + `TASK.md` | ⬜ **다음** | — |
...
### Step 8 착수 시 할 일 (구체) — TASK M9
- `TASK/00-빌드순서-마일스톤.md`: 마일스톤 **M9(참여·로그인)** 추가 + Tier-0 게이트 **AU-1~4**(#28~31) 배선. 빌드순서에 SP-DB(참여 7테이블 DDL)·SP-AUTH·SP-INFRA(메일존) **선행 관계** 반영(§C).
- `TASK/13-로그인참여.md`(신규): SP-AUTH-1~15를 TDD 리프로 분해. **최우선 리프 T-13.2.1**(run_tests.sh 참여 7테이블 백업 확장, §C)이 실 DB 게이트 전 선행.
- `TASK.md` 인덱스 3요소 갱신. **라이브 배포·정책 라이브 반영은 AdSense 심사 후**(§E).
