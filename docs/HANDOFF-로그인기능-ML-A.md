...
> **다음 세션 시작점**: 아래 §진행현황의 **Step 7(SPEC/09 정책 — 약관 T5 신설·T2 정정·개인정보 P7 신설)** 부터. 이 문서 + 메모리 `loupit-login-feature-docs`가 재개 컨텍스트다.
> **최종 갱신**: 2026-07-21, ML-A 6(SPEC/13 SP-AUTH) 저술·적대적 검증(blocker 0, major 3 수정) 완료 시점.
...
| 7 | `SPEC/09-정책-페이지.md`(약관 **T5 신설** + **T2 정정** + 개인정보 **P7 신설**) | ⬜ **다음** | — |
...
### Step 7 착수 시 할 일 (구체) — SPEC/09 정책
- **약관 T5 신설**: 탈퇴 후 로그인 이메일 파기·닉네임/편집이력 존치 고지(FR-104·UC-72가 이미 T5 인용). 앵커 `#t5`.
- **약관 T2 정정**: 현 T2 "본 서비스는 로그인·회원가입 없이 제공…"(SPEC/09 L210)이 SC14와 정면 모순 → "기여 시 무비밀번호 로그인 제공, 익명 열람은 무로그인"으로 개정.
- **개인정보 P7 신설**: 회원 PII(로그인 이메일·닉네임)·수집이용·탈퇴 시 이메일 파기·닉네임/이력 존치·보관기간 고지(현 SPEC/09는 P1~P6뿐). SPEC/13이 이미 P7을 인용(Step 7 신설 대기로 표기).
- REQUIRED_ITEMS·PC 검증표·SP-POL 마스터표에 T5·P7 반영. **라이브 `/privacy`·`/terms` 반영은 AdSense 심사 후**(§E).
...
### 흡수된 항목(별도 작업 아님)
- ML-A 1~5 major 중 'SP-AUTH 파일 미작성' → **Step 6이 해소**. '약관 T5 미존재·T2 모순' → **Step 7이 해소**(T5 신설·T2 정정).
