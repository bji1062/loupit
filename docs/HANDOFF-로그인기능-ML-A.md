...
> **다음 세션 시작점**: **ML-B ①·② 완료·검증(✅)** — ① SP-DB(SPEC/02 SP-DB-17), ② SP-INFRA(SPEC/11 §3.4·§6.2a·§7, 적대검증 2회). 다음 = **③ Tier-0 test 재명세**(§F 진행표)[§C 정정 4건 + **③ RED 스테이징 설계 결정 필요** — 라이브 릴리스 게이트(`run_tests.sh`) 건드림, SC14 마커/스킵 격리] → ④ T-13.2.1 백업 확장(§C·§F: **최우선 선행** — ③보다 먼저가 안전). ③/④는 실 코드(`server/tests/`·`infra/deploy/`) 변경이라 **착수 전 사용자 확인 권장**. → **검토 체크포인트 정지**. M9 TDD(TASK/13 41리프)는 검토·승인 후. **배포는 AdSense 심사 결과까지 홀드**(§E). 이 문서 + 메모리 `loupit-login-feature-docs`가 재개 컨텍스트다.
