...
| **OI-1a** | **엔씨소프트** — 복지 데이터 있으나 200 목록(KOSPI/KOSDAQ 시총 상위)에 **없음**. 추가(201번째)할지 vs 미사용 | 사용자 결정 대기 |
| **OI-1b** | **CJ.sql 오매핑** — 파일명 "CJ"지만 실제 데이터는 **CJ올리브네트웍스(비상장 계열사)**, 200목록 "CJ ENM"과 다른 회사. CJ ENM에 붙이면 오류(DEC-2 official 위반) | 사용자 결정 대기 |
