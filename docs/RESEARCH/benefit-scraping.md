...
> loupit 불변 제약: 로그인 없음 · 읽기전용 백엔드 · 빌드타임 정적 생성 · 공개 회사 페이지 = 복지 데이터가 있는 회사(~96개)만(법적 리스크 R3) · 큐레이션 96개 SQL 재사용.
...
2. **회사 ~96개 × 제각각 구조 = 정적 셀렉터 파싱 불가.** JS 렌더·이미지 인포그래픽이 많아 requests+BS4는 커버리지 미달, "~96개 × 셀렉터"는 유지보수 지옥. **LLM 스키마 강제 파싱이 유일한 유지보수 가능 경로**이며, 기존 96개가 이미 이 방식(AI 파싱)으로 생산된 검증된 경로다.
...
| **폴백2** ~~(회사페이지 폴백)~~ | 기업유형 프리셋(`TBENEFIT_PRESET`) | **회사페이지 폴백 아님(2026-07-09 결정)**: 등록 회사는 모두 실복지 보유 → 프리셋으로 채우지 않음. 프리셋은 **비교 툴 "직접 입력 모드"**(사용자 회사가 등록 ~96개에 없을 때 유형 기반 기본 복지 템플릿, UC-15/21)용. `est` | 자체 큐레이션 상수, 이슈 없음 |
...
```
[0] 회사 목록(복지 데이터가 있는 회사 ~96개; 메타는 기존 200 시드에서 매칭, D2.3)
...
[3] LLM 파싱   Anthropic SDK 배치 client.messages.parse(output_config.format=JSONSchema)
              ...
              환각 억제: evidence_quote 비면 폐기 / 원문에 회사 없으면 빈 배열(→ 해당 회사 미등록; 복지 실데이터 있는 회사만 등록)
...
[8] SQL/DB 시드  sql/{회사}.sql (INSERT IGNORE TCOMPANY + DELETE WHERE BADGE_CD='est'(official 보존)
              ...
              실데이터 없는 회사는 미등록(등록 회사는 모두 실복지 보유 → 회사페이지 프리셋 폴백 불필요; TBENEFIT_PRESET는 비교 툴 직접 입력 모드용)
[9] 정적 생성  DB → 회사 상세 ~96개 정적 HTML(JS 없이 본문, UC-44) + 인기 조합(F5)
...
| 단계 | 범위 | 산출물 | 대략 공수 |
| --- | --- | --- | --- |
| **Phase 0 — 재이식 + 백필** (MVP 필수) | 96개 SQL → `TCOMPANY_BENEFIT` 로더. `BADGE_CD=official`(DEC-2)·`amt_source`·출처·TTL 백필 | 등록 회사 ~96개 전부 실복지 보유(프리셋 폴백 없음; 복지 데이터가 있는 회사만 등록) | 2~3일 |
| **Phase 1 — 정적 생성 + 론칭** (MVP 필수) | DB 시드 → 회사 ~96개 정적 HTML + reference/all + sitemap. 면책·정책(F7) | AdSense 승인 콘텐츠(R1), 색인 자산(R4) | 3~5일 |
...
**핵심 판단: Phase 0+1만으로 MVP 론칭 가능.** 96개가 D2.4 보존 대상이고 등록 회사(~96개)는 모두 실복지 보유(빈 회사 없음)이므로, 신규 LLM 스크래핑(Phase 2)은 **론칭 차단요소가 아니라 갱신·확충 도구**다. "코드는 새로, 데이터는 보존" 원칙과 정합.
...
| ID | 이슈 | 비고 |
| --- | --- | --- |
| ~~OI-1~~ ✅ | 96개↔200개 교집합 **검증 완료(2026-07-09)**: 94/96 정확 매칭. 예외 2건 → OI-1a·OI-1b | 아래 |
| ~~OI-1a~~ ✅ | **엔씨소프트** — 복지 데이터 있으나 200 시드 목록(KOSPI/KOSDAQ 시총 상위)에 없음. **결정(2026-07-09): 등록 포함**(시드 메타는 별도 확보) | 확정 |
| ~~OI-1b~~ ✅ | **CJ.sql 오매핑** — 파일명 "CJ"지만 실제 데이터는 **CJ올리브네트웍스(비상장 계열사)**, 200 시드 "CJ ENM"과 다른 회사. **결정(2026-07-09): 회사를 "CJ올리브네트웍스"로 등록**(CJ ENM 아님, 혼동 금지) | 확정 |
