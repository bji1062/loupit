...
loupit FLOW 문서군은 USECASE·FRD가 정의한 화면과 화면 사이의 전이를 **안정 ID(SCR-*·FLOW-*)**로 확정하는 문서군이다. 8개 문서가 사이트맵·전역 네비게이션과 기능 대역별 화면·플로우(랜딩·검색·입력·리포트·회사상세/조합·정책/광고/동의·오류/엣지)를 나눠 소유한다. 상위 추적은 FLOW → FRD([FRD.md](FRD.md)) → USECASE → PRD → 브리프이며, 프로파일러(가치관 진단 설문)와 익명 서버 쓰기 화면은 제품 범위에서 영구 제외되며, 로그인·회원·계정은 복지 등록·수정 기여(SC14, 문서 09/13·SP-AUTH) 한정 In-scope이나 이 익명 플로우엔 어떤 화면·라우트·전이에도 등장하지 않는다(FR-01).
...
- **커버리지**: 사이트의 모든 사용자 화면(랜딩·비교 툴 3뷰·회사 상세·인기 조합·정책 4종·404)이 SCR-01~09로 식별되고, 각 기능 대역의 상세 화면(SCR-1x~SCR-9x)이 이를 분해·인용한다. 프로파일러·익명 서버 측 사용자 쓰기 화면은 어떤 라우트·메뉴·전이에도 없다; 로그인·회원 화면은 복지 기여 SC14 한정이라 이 익명 흐름엔 비등장(FR-01).
