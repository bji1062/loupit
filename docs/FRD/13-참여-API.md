...
**상세 동작**
- 정상: 요청 이메일에 대해 6자리 코드를 생성, 코드의 **해시**만 만료(+10분)와 함께 저장(TAUTH_CODE, PURPOSE=login)하고 이메일로 발송한 뒤 **균일 204**를 반환한다.
...
2. 코드 원문은 저장하지 않고 해시만(TAUTH_CODE.CODE_HASH_VAL). 만료 +10분(FR-111).
...
| 410 Gone | 코드 만료(+10분) | 오류 envelope |
...
2. **로그인/인증 코드**: 원문 무저장, 해시만(TAUTH_CODE.CODE_HASH_VAL), 만료 +10분, 소비 후 무효.
...
2. **코드 검증**: 시도 상한(5회, TAUTH_CODE.ATTEMPT_CNT)·만료(10분). 초과 시 코드 무효화·429.
