...
- 예외(도메인 불일치): 입력 이메일 도메인이 회사 등록 도메인과 다르면 422. **응답 `detail` 은
  `{"code": "domain_mismatch", "domains": [등록 도메인…]}` 구조**다(2026-07-29).
  - 왜 구조인가: 회사에 도메인이 **하나라도** 등록되면 그 회사는 위 409 `manual_required`
    폴백을 잃는다. 그 순간부터 계열사·자회사 주소를 쓰는 재직자는 갈 곳이 없어진다 —
    도메인 커버리지를 넓힐수록 넓어지는 막다른 길이다. 프론트가 "현재 등록된 도메인은 …"
    을 보여주고 수동 승인(FR-107)으로 이으려면 등록 도메인이 기계가 읽는 형태여야 한다.
    UC-2a 가 처음부터 "필요 시 UC-74(수동 승인) 안내"로 규정했던 동작을 구현이 뒤늦게 따른다.
  - `code` 를 함께 싣는 이유: FastAPI 자체 검증 실패도 422 인데 그쪽 `detail` 은 **리스트**다.
    구조로만 갈라도 되지만, 의도를 코드에 남겨 오분류를 막는다(입력 형식 오류를 수동 승인으로
    보내면 사용자는 고칠 수 있는 것을 고치지 못한다).
  - 노출 판단: 등록 도메인은 애초에 회사 공식 사이트에 게시된 주소에서 확증한 공개 정보이고,
    도메인을 안다고 그 주소로 오는 메일을 받을 수는 없다 → 인증 강도가 낮아지지 않는다.
...
| 422 Unprocessable Entity | 입력 도메인이 회사 등록 도메인과 불일치 — `detail={code:"domain_mismatch", domains:[…]}` (프론트는 수동 승인 폴백으로 잇는다) |
