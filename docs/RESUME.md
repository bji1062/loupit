...
> ⚠️ **최신 상태는 [`docs/HANDOFF-2026-07-19.md`](HANDOFF-2026-07-19.md)가 정본이다.**
> 이 문서는 2026-07-12 시점(컷오버 전) 기록으로, 프로젝트 배경·규칙 참고용이다.
> 다음 세션은 HANDOFF부터 읽어라.
...
>
> **🎨 디자인 비주얼 폴리시 완료(2026-07-13)**: 토큰만 있던 `styles.css`에 전 표면 스타일 작성. 초기 "시안 A 라이트·블루" 후 **사용자가 Claude.com/Anthropic 시스템 지정 → 최종 채택**: **크림 캔버스 `#faf9f5`+코랄 `#cc785c`+다크네이비 `#181715`+잉크 `#141413`, 세리프 디스플레이 헤드라인**(외부폰트 CDN 금지 → 시스템 세리프 스택, 한글 명조 폴백; 본문 자체호스팅 sans), 다크 푸터(크림→다크 리듬). 컴포넌트는 `var(--*)`만(UT-NO-LITERAL), `tokens.test.js` UT-LIGHT-DEFAULT, WCAG AA 62/62. 회사(코랄 콜아웃)·랜딩·비교툴 3뷰·리포트(코랄틴트 판정카드)·정책·배지 적용, beta 라이브 gstack 검증 완료. **⚠️ beta 자산 캐시**: 원래 immutable 1년이라 갱신이 브라우저 하드캐시에 막혔음 → beta.conf 자산 `Cache-Control: no-cache`로 변경(`a7cc630`). 기존 캐시된 브라우저는 **1회 하드리프레시(Ctrl+Shift+R)** 후부터 자동 최신. **완성도 정정**: "코드 ~96%"는 배선 갭으로 과대였고, 배선+디자인 후 **실제 동작·실제 제품 외관** 확보. 남은 것 = MD-2~6 접근성 QA·MB-7 직접입력·AdSense 승인·🚦컷오버.

- **git**: `origin/main @ 6336c9c` (push 완료·동기화, 작업트리 clean). 이 세션 흐름: §B 1~7 결함수정(`624223d`~`b2a8bc7`) → **비교툴 인터랙티브 배선 계층 구현**(런치블로커 수정 `cd20712`, ui.js+jsdom통합테스트) → **디자인**: 시안 A 라이트(`0e8ba57`) 후 **Claude.com/Anthropic 시스템 채택**(크림·코랄·세리프 `88427c1`) → **버터플라이 차트**(예전 loupit 이식 `6336c9c`) → beta 자산캐시 immutable→no-cache(`a7cc630`). 다른 PC에서 `git pull`로 이어받기.
...
- **⚠️ 테스트 안전규칙(C-1, 2026-08-26 격리 전환 후)**: 테스트는 격리 스키마 `loupit_test` 만 쓴다 — **`bash infra/deploy/run_tests.sh`** 또는 `DB_NAME=loupit_test LOUPIT_ALLOW_FRESH=1 python3 -m pytest server/tests`. 서빙(LOUPIT)을 겨눠 맨 `pytest`로 DB 테스트를 돌리면 conftest 가드가 `[C-1 안전장치]`로 차단한다. **`LOUPIT_ALLOW_SERVING_SCHEMA=1` 설정 후 맨 pytest 실행 절대 금지**(서빙 비운 채 복원 안 됨). 서빙 깨지면 수동 복구: **백업 복원이 먼저**(`infra/deploy/restore.sh /var/backups/loupit/loupit-<날짜>.sql.gz` — 재직자 행·편집 이력이 들어 있다), 백업이 없을 때만 `python3 db/seed/load.py`(멱등 재적용 — 재직자 행은 그대로 둔다. 표가 DROP·재생성됐다면 새 행이 편집 이력의 번호를 받게 되므로 거부된다) + `sudo systemctl restart loupit-api loupit-beta-api`(양쪽 참조 캐시 무효화, 동일 서빙 DB). ⚠ 2026-09-24 갱신: `--fresh` 는 재직자 데이터(편집 이력·재직자 행)가 있으면 거부되고, 폐기 허용은 서빙 스키마 이름이면 무조건 거부된다(SP-SEED-12).
