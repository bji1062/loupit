...
- [ ] 서버의 리포가 최신 커밋을 갖도록: `cd /home/ubuntu/loupit && git pull`
...
```bash
cd /home/ubuntu/loupit
bash infra/deploy/release.sh   # schema→seed→generate→테스트 게이트→API 재시작→nginx reload→스모크
```

빠른 부분 배포만 원하면(스키마·시드 무변경 시):

```bash
server/venv/bin/python -m generator.build --out web/dist
...
- `release.sh`는 테스트 게이트 실패 시 [5/7] 이후를 실행하지 않으므로, 실패 시 서비스는 이전 산출물 그대로다.
