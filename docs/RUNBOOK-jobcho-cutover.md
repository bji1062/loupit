...
- `release.sh`는 재구조화(2026-07-18) 후 **테스트 게이트가 [1/7]로 맨 앞**이라, 게이트 실패 시
  ...
  실패 시 큰 경고와 함께 수동 복구를 안내한다(`python3 db/seed/load.py` — 멱등 재적용, 빈 테이블도
  채운다 + `sudo systemctl restart loupit-api loupit-beta-api`). ⚠ `--fresh` 는 쓰지 않는다 — 재직자
  데이터(편집 이력·재직자 행)가 있으면 로더가 거부한다(SP-SEED-12, 2026-09-24). (게이트는 2026-08-26
  격리 전환 뒤로 서빙을 비우지 않는다 — 이 문단의 창은 구 판본 이야기다.) build/스왑([4/7]) 이후 단계 실패는 새 정적물이
