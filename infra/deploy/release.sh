...
_on_err() {
  local ec=$?
  {
    ...
    echo "     90 미만(빈/불완전)이면 백업 복원이 먼저다 — 재직자 행·편집 이력이 들어 있다:"
    echo "       infra/deploy/restore.sh /var/backups/loupit/loupit-<날짜>.sql.gz"
    echo "     백업이 없을 때만 멱등 재적재: ${PY} db/seed/load.py (재직자 행은 그대로 둔다)."
    echo "       표가 DROP·재생성됐다면 이 적재는 거부된다 — 새 행이 편집 이력의 번호를 받게 되므로(SP-SEED-12 ⑦)."
