...
# BASE=https://loupit.co(기본, 배포) 또는 BASE=http://127.0.0.1(로컬 스택 검증) 환경변수로 오버라이드.
set -uo pipefail
BASE="${BASE:-https://loupit.co}"
fail=0
...
chk "SM-2 http->https 301"  "[ \"\$(curl -s -o /dev/null -w '%{http_code}' http://loupit.co/)\" = 301 ]"
...
# ── TLS 유효성(SM-10) ──
chk "SM-10 tls valid" "echo | openssl s_client -connect loupit.co:443 -servername loupit.co 2>/dev/null | openssl x509 -noout -checkend 0"
